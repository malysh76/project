...
/*==================================================================================*/
unsigned char TxMsg[255];
unsigned char RxMsg[255];
...
unsigned char 			WAKE_GetMsg 		(UART_TypeDef * pUART,
											 unsigned char *pCmd,
											 unsigned char* pMsg,
											 unsigned char* pMsgLen);
void 					WAKE_PutMsg			(UART_TypeDef * pUART,
											 unsigned char cmd,
											 unsigned char* pMsg,
											 unsigned char msg_len);
static void 			WAKE_PutErrMsg		(UART_TypeDef * pUART,
											 unsigned char err);
static unsigned char 	WAKE_GetPacket 		(TWAKE_STATUS* pStatus,
											 TWAKE_PACKET* pPacket,
											 unsigned char data);
static void 			WAKE_PutPacket		(UART_TypeDef * pUART,
											 TWAKE_PACKET* pPacket);
...
/*==================================================================================*/
unsigned char WAKE_GetMsg (UART_TypeDef * pUART, unsigned char *pCmd, unsigned char* pMsg, unsigned char* pMsgLen)
{
// Локальные переменные
...
// Определение указателя на тело сообщения
	RxPacket.Msg = pMsg;
// Обработка буфера
//	while (!UART_EMPTY(pUART))//	CDC_EMPTY()
	while (!UART_EMPTY())
	{
		//WDT_Reset();
	// Чтение данных из буфера порта
//		data = UART_GetChar(pUART);
		data = UART_GetChar();
		//data = CDC_GetChar();
		...
		switch (WAKE_GetPacket(&RcvStatus, &RxPacket, data))
		{
			...
			// Проверка признака сообщения
				if(RxPacket.sign == 0x01)
					WAKE_PutPacket(pUART, &TxPacket); // повтор пакета в случае ошибки
				else
				{
			...
			// Отправка сообщения об ошибке пакета
				WAKE_PutErrMsg(pUART,  ERR_STAFF);
			break;

			case 2:
			// ошибка пакета
			// Отправка сообщения об ошибке пакета
				WAKE_PutErrMsg(pUART,  ERR_SIGN);
			break;

			case 3:
			// Отправка сообщения об ошибке пакета
				WAKE_PutErrMsg(pUART,  ERR_LEN);
			break;
			case 4:
				// ошибка crc
			// Отправка сообщения об ошибке пакета
				WAKE_PutErrMsg(pUART,  ERR_CRC);
			break;
...
/*	Выходные параметры:	нет															*/
/*==================================================================================*/
void WAKE_PutMsg (UART_TypeDef * pUART, unsigned char cmd, unsigned char* pMsg, unsigned char msg_len)
{
// Локальные переменные
// Программа
...
/*	Выходные параметры:	нет															*/
/*==================================================================================*/
static void WAKE_PutErrMsg (UART_TypeDef * pUART, unsigned char err)
{
// Локальные переменные
// Программа
// Инициализация процесса передачи
...
/*	Выходные параметры:	нет															*/
/*==================================================================================*/
static void WAKE_PutPacket (UART_TypeDef * pUART, TWAKE_PACKET* pPacket)
{
// Локальные переменные
...
//
	...
	unsigned char	TxBuf[128];						// промежуточный буфер
// Программа
// Передача сообщения
	while(tran_sign)
	{
	...
	// Отправка данных в порт
		...
		{
			UART_PutData(TxBuf, tx_cnt);
		//CDC_PutData(pUART, TxBuf, tx_cnt);
